...
export type CustomLoaderProps = {
	...
	size?: number;
	/** Fill color for loader paths. */
	color?: string;
	className?: string;
	/** Styles for the outer flex container. */
	style?: CSSProperties;
	/** Prefer `style`. Kept for backward compatibility; `style` overrides on conflict. */
	containerStyle?: CSSProperties;
	svgClassName?: string;
	svgStyle?: CSSProperties;
	/** Accessible name; parent should set when context differs (e.g. "Saving board"). */
	"aria-label"?: string;
};
...
export function CustomLoader({
	...
	return (
		...
		>
			<svg
				className={cn("layers-loader", svgClassName)}
				width={size}
				height={size}
				viewBox="0 0 32 32"
				fill="none"
